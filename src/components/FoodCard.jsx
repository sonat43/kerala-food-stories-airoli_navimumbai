...
export default function FoodCard({ dish }) {
  ...
  return (
    <Motion.article
      ...
      className="group flex min-h-[430px] flex-col overflow-hidden rounded-[1.5rem] border border-[#e8dfd4] bg-paper shadow-[0_10px_30px_rgba(43,35,29,0.05)] transition-all hover:-translate-y-1 hover:border-clay/20 hover:shadow-card"
    >
      <div className="relative h-60 w-full shrink-0 overflow-hidden bg-[#e6ded2] sm:h-64">
        {!imageFailed ? (
        ...
        <div className="absolute inset-x-5 bottom-5 flex items-end justify-between gap-2">
          <span className="rounded-full border border-white/20 bg-teak/55 px-2.5 py-1.5 text-[8px] font-semibold uppercase tracking-[0.16em] text-white/85 backdrop-blur-sm">Made fresh</span>
          <span className="font-display text-sm italic text-white/80">Naatile taste</span>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h3 className="font-display text-[1.35rem] font-semibold leading-[1.04] tracking-[-0.02em] text-teak sm:text-[1.5rem]">{dish.name}</h3>
            <p className="mt-1.5 text-[11px] text-teak/42" lang="ml">{dish.malayalamName}</p>
          </div>
        </div>

        <p className="mt-3 line-clamp-2 text-xs leading-5 text-teak/55 sm:text-[13px]">{dish.description}</p>

        {dish.includedItems && (
          <div className="mt-3 rounded-xl bg-clay/[0.05] p-2.5">
        ...
        {dish.parcelCharge > 0 && <p className="mt-2 text-[10px] font-semibold text-clay/70">+ ₹{dish.parcelCharge} parcel charge</p>}

        <div className="mt-auto flex items-end justify-between gap-2 border-t border-sand/70 pt-3">
          ...
          <div className="flex items-center gap-3">
            ...
            {quantity === 0 ? (
              <button onClick={() => addItem(cartItem)} className="flex h-11 items-center gap-1.5 rounded-full bg-clay px-5 text-xs font-bold text-white shadow-sm shadow-clay/15 transition hover:bg-clay-dark active:scale-95"><Plus size={14} /> Add to order</button>
            ) : (
